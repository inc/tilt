...
def import_unused(currency, addrs):
    req = {
        'wallet': get_config("wallet_id"),
    ...
    r = requests.post('https://tilt.cash/api/v1/import_unused', json=req)
    try:
        return(r.json())
    except:
        logging.error("request failed; is tiltwlt running?")

def list_unused(currency):
    req = {
        'wallet': get_config("wallet_id"),
        'api_key': get_config("api_key"),
        'currency': currency
    }
    r = requests.post('https://tilt.cash/api/v1/list_unused', json=req)
    try:
        return(r.json())
    except:
        logging.error("request failed; is tiltwlt running?")

def destroy_unused(currency):
    req = {
        'wallet': get_config("wallet_id"),
        'api_key': get_config("api_key"),
        'currency': currency
    }
    r = requests.post('https://tilt.cash/api/v1/destroy_unused', json=req)
    try:
        return(r.json())
    except:
        logging.error("request failed; is tiltwlt running?")
